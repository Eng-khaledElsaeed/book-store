...
function category_fill(page=1,search_value=""){
            ...
            if(status){
                if(data.length >0){
                    data.forEach((item)=>{
                        ...
                        <td><button type='button' id='update_btn-${item.category_id}' class='update_btn' data-value='category' data-id='${item.category_id}' onclick='updateElement(${item.category_id});'>update</button></td>
                        <td><button type='button' id='delete_btn-${item.category_id}' class='delete_btn' data-value='category' data-id='${item.category_id}' onclick='deleteElement(${item.category_id});'>delete</button></td>
                        </tr>
                        `
                    });
    ...
    xhr.open("GET","ajax_category_search.php?search_value="+search_value+"&page="+page,true);
    xhr.send();
}
category_fill();
